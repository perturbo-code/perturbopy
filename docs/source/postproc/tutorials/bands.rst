...
* ``cartesian``: coordinates are in units of 2 pi / alat. 
...
The direct bandgap is the difference between the maximal-energy state of the valence band and the minimal-energy state of the conduction band, for which the k-vectors are the same. For example, to compute the direct bandgap in silicon between the valence band (band index 4) and conduction band (band index 5), we call the ``direct_bandgap`` method with the two band indices as inputs:
...
The indirect bandgap is the difference between the maximal-energy state of the valence band and the minimal-energy state of the conduction band, for which the k-vectors differ. For example, to compute the indirect bandgap in silicon between the valence band (band index 4) and conduction band (band index 5), we call the ``indirect_bandgap`` method with the two band indices as inputs:
...
The ``indirect_bandgap`` method returns the bandgap, 0.458 eV, the k-point of the maximal-energy state of the valence band, [0, 0, 0], and the k-point of the minimal-energy state of the conduction band, [0.43137, 0.     , 0.43137].
...
Another important input is ``max_distance``, which is the maximum distance from the central k-point to other k-points included in the calculation. For example, let's compute the longitudinal effective mass at [0.43, 0., 0.43], which is the CBM of silicon. We will use max_distance of 0.12. The experimental value is about 0.98. 
...
The plot created shows the bands, with the points selected for the approximation plotted in red. Note that the points and line of fit stop at the "X" point because past here, the effective mass is no longer longitudinal.
...
.. image:: figures/silicon_el_effective_mass_zoom.png
	:width: 400
	:align: center
To increase the number of points used in the calculation, we should increase max_dist. (show example?)
...
.. image:: figures/silicon_hole_effective_mass_zoom.png
	:width: 450
	:align: center
...
Note that k-point labels can be removed from the plot by setting the ``show_labels`` input (default True) to False.
