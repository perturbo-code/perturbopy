...
In several calculation modes, we work with sets of points in reciprocal space, such as k-points and q-points. These are stored in  objects of type :py:class:`.RecipPtDB`. For example, let's look at the ``kpt`` attribute of the :py:class:`.Bands`, which stores the k-points used in the ``bands`` calculation.
