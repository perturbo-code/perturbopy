...
def create_arg_namespace(param_data, input_data):
    ...
    for key, value in param_data['perturbo'].items():
        ...
        if 'default' in value:
            description = f'{description} Default value is {value["default"]}.'
        if 'typical' in value:
            description = f'{description} Typical value is {value["typical"]}.'
        parser.add_argument('--' + key, metavar=f'type={value["type"]}', help=description)
...
def input_generation():
    ...
    general_arg_list = ['calc_mode', 'file_name', 'prefix']
    ...
    if not args.file_name:
        if args.calc_mode == 'qe2pert':
            file_name = 'qe2pert.in'
        else:
            file_name = 'pert.in'
    else:
        file_name = args.file_name

    finput = open(file_name, 'w')
    ...
    print('File ' + file_name + ' is generated.')
